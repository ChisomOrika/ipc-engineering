...
├── generate_weekly_activation_report.py   4. Weekly PPTX report generator
...
### Report generator (`generate_weekly_activation_report.py`)

The one recurring report this repo produces is the **weekly activation report** — a 7-slide PPTX covering DAASH + GoSource performance vs a 4-week trailing average.

| Script | Output | Cadence | Week definition |
|---|---|---|---|
| `generate_weekly_activation_report.py` | `IPC_Weekly_Report_<date>.pptx` (on Desktop) | Weekly | Last Fri → This Thu, with 4-week trailing avg |
...
Monthly reports and historical one-offs (March/April/GoSource-specific decks) were previously generated by separate scripts that were removed during handoff cleanup. If you need to recreate a monthly deck, the activation report is a good template to copy and adapt — the SQL patterns and `python-pptx` styling are reusable.
...
| Weekly report output | `IPC_Weekly_Report_YYYYMMDD.pptx` (written to Desktop) | — |
